...
public:
	...
	void sendCellData(int cellHovered, bool clicked);

	bool isThisPlayerTurn();
	bool isAPlayer(); // not in use.
	bool isPlayerOne();
	bool isPlayerTwo();
	void beginMatch(); // initialize the game and decide which player goes first with "dice".
	void switchPlayer(); // it's next player's turn

private:
	bool m_isReady = false; // this is ready to start a match.
	bool m_isReady2 = false; // the other player is ready to start a match.
	int m_thisPlayerNr = -1; // the playerNr of this instance.
	int m_player1Nr;
	int m_player2Nr;
	int m_currentPlayerNr; // the id (playerNr) of the current player to make its turn in the tic tac toe match.
